...
const ChatsPage = () => {
  const [
    ...
    singleChatOffCanvasShow,
    singleChatOffCanvasHandleShow,
    singleChatOffCanvasHandleClose,
    usersData,
    usersDataIsLoading,
    offCanvasSreachInputValue,
    offCanvasInputOnChangeHandle,
    userCardOnClickHandle,
    createSingleChatResponse,
    createSingleChatResponseIsLoading,
    userCardonClickId,
    chatCardOnClickHandle,
    chatCardonClickId,
    chatCardRefs,
    chatCardActiveStatus,
    chatData,
    chatsContainerRef,
    endOfDataStatus,
    groupChatOffCanvasShow,
    groupChatOffCanvasHandleShow,
    groupChatOffCanvasHandleClose,
    offCanvasGroupNameInputValue,
    offCanvasGroupNameInputOnChangeHandle,
    offCanvasGroupNameInputRef,
    deleteUserIconOnClickHandle,
    offCanvasCreateGroupChatOnClickHandle,
    groupChatSearchResultsUserCardOnClickHandle,
    groupChatSelectedUsers,
    groupChatSelectedUsersId,
    createGroupChatResponse,
    createGroupChatResponseIsLoading,
    updatingDispatchLoader,
    clickedChatCardData,
    chatUserInfoIconOnClickHandle,
    updateGroupNameButtonOnClickHandle,
    renameGroupChatResponseIsLoading,
    updateGroupChatIconIsClicked,
    removeUserFromGroupChatResponseIsLoading,
    addUserToGroupChatResponseIsLoading,
    getAllChatMessagesData,
    getAllChatMessagesDataIsLoading,
    sendMessageInputValue,
    sendMessageInputOnChangeHandle,
    sendMessageInputRef,
    sendMessageInputOnKeyDownHandle,
    sendMessageOnClickHandle,
    messageCardImageOnClickHandle,
    ...
    setChatData,
    setChatCardIsClicked,
    setChatCardonClickId,
    setChatCardActiveStatus,
    setClickedChatCardData,
    groupChatMessages,
    setAllChatMessages,
    createMessageResponseIsLoading,
    setLastSeen,
    momentDateAndTimeFormat,
  ] = ChatsPageHook();
  ...
  return (
    <div className="chats-page">
      ...
      <div className="chats-page_body row">
        <div
          className={`chats-page_body-left col-md-6 col-lg-5 col-xl-4 ${
            isMobileSize === true ? "d-none" : ""
          }`}
        >
          <ChatContainer
            title="Chats"
            icons={
              <div className="55">
                <i
                  ...
                  title="Group Chat"
                  onClick={groupChatOffCanvasHandleShow}
                ></i>
                <i
                  className="fa-solid fa-user-plus"
                  title="Single Chat"
                  onClick={singleChatOffCanvasHandleShow}
                ></i>
              </div>
            }
            containerRef={chatsContainerRef}
            content={
              (allUserChatsDataIsLoading && !chatData.length) ||
              updatingDispatchLoader ? (
                <div className="55">
                    ...
                    return (
                      <UserCard
                        ...
                        content={
                          <span
                            className={`${
                              chatCardonClickId === item._id ? "" : ""
                            } ${
                              item.latestMessage?.message ? "" : "invisible"
                            }`}
                          >
                            <span
                              className={`${
                            ...
                            >
                              <span
                                className={`fa-solid fa-check me-1 text-secondary ${
                                  item.latestMessage?.seenStatus ===
                                    "delivered" ||
                                  item.latestMessage?.seenStatus === "seen"
                                    ? "d-none"
                                    : ""
                                }`}
                              ></span>
                              <span
                                className={`fa-solid fa-check-double me-1 ${
                                  item.latestMessage?.seenStatus ===
                                    "delivered" ||
                                  item.latestMessage?.seenStatus === "seen"
                                    ? ""
                                ...
                                title={item.latestMessage?.seenBy?.map(
                                  (item) => item.name
                                )}
                              ></span>
                            </span>
                            ...
                            {item.latestMessage?.message.length >= 11 &&
                            item.isGroupChat
                              ? `${item.latestMessage?.message.slice(0, 11)}...`
                              : item.latestMessage?.message.length >= 20 &&
                                !item.isGroupChat
                              ? `${item.latestMessage?.message.slice(0, 20)}...`
                              : item.latestMessage?.message}
                            <span
                              className={`latest-message_date-and-time ${
                                (specificChatDataIsLoading ||
                                  getAllChatMessagesDataIsLoading) &&
                                item._id === chatCardonClickId
                                  ? "d-none"
                                  : ""
                              }`}
                            >
                ...
                <span
                  className="fa-solid fa-user-plus text-primary d-flex flex-column justify-content-center align-items-center h-100 user-select-none shadow-none fs-1 opacity-50"
                  onClick={singleChatOffCanvasHandleShow}
                >
        ...
        <div
          className={`chats-page_body-right col-md-6 col-lg-7 col-xl-8 ${
            isMobileSize === true ? "d-block" : ""
          }`}
        >
          <ChatContainer
            title={
              <div className="d-flex flex-column">
                ...
                {typingData?.isTyping && clickedChatCardData?.isGroupChat ? (
                  ...
                  <span className={`chat-typing-indicator`}>
                    {groupOnlineUsers?.length === 0
                      ? "No Active Users Now"
                      : groupOnlineUsers?.length === 1
                      ? "1 Active User"
                      : `${groupOnlineUsers?.length} Active Users`}
                ...
                ) : onlineUsersData?.includes(clickedChatCardData?.users._id) &&
                  !clickedChatCardData?.isGroupChat ? (
                  <span className={`chat-typing-indicator`}>Online</span>
                ) : null}
                <span
                  className={`chat-typing-indicator ${
                    !onlineUsersData?.includes(
                    ...
                    chatCardIsClicked
                      ? ""
                      : "d-none"
                  } ${specificChatDataIsLoading ? "d-none" : ""}`}
                >
                  <span className="last-seen-span">Last Seen:</span> {lastSeen}
                </span>
              </div>
            }
            ...
            backIcon={
              <i
                className="fa-solid fa-arrow-left ms-0 me-3"
                onClick={() => {
            ...
            icons={
              <i
                ...
                onClick={chatUserInfoIconOnClickHandle}
              ></i>
            }
            ...
            content={
              <>
                {getAllChatMessagesDataIsLoading ? (
                  ...
                  <ScrollableFeed>
                    {Object.entries(allChatMessages)?.map(
                      ([key, value], index, obj) => {
                        return (
                          <div key={key}>
                            ...
                            <span
                              className={`send-message_spinner spinner-border ${
                                createMessageResponseIsLoading &&
                                index === obj.length - 1
                                  ? ""
                                  : "invisible"
                  ...
                  chatCardIsClicked ? (
                  <span
                    className="fa-solid fa-feather d-flex flex-column justify-content-center align-items-center h-100 text-primary opacity-50 user-select-none fs-1"
                    onClick={() => {
                ...
                <div
                  className={`chats-page_send-message-input d-flex justtify-content-center align-items-center ${
                    Object.entries(allChatMessages)?.length ||
                    (!Object.entries(allChatMessages)?.length &&
                      chatCardIsClicked)
                      ? ""
                ...
                >
                  <input
                    ...
                    onChange={sendMessageInputOnChangeHandle}
                    onKeyDown={sendMessageInputOnKeyDownHandle}
                    ref={sendMessageInputRef}
                  />
                  <button
                    className="fa-solid fa-paper-plane"
                    onClick={sendMessageOnClickHandle}
                  ></button>
                </div>
              </>
            }
